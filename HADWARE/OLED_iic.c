...
void I2C_Configuration(void)
{
  ...
  I2C_InitStructure.I2C_OwnAddress1 =OLED_ADDRESS;
...
void OLED_ShowPicture(unsigned char x, unsigned char y, unsigned char N)
{
	unsigned char wm=0;
	unsigned int  adder=48*6*N;
	OLED_SetPos(x , y);
	for(wm = 0;wm < 48;wm++)
	{
		WriteDat(F48x48[adder]);
		adder += 1;
	}
	OLED_SetPos(x,y + 1);
	for(wm = 0;wm < 48;wm++)
